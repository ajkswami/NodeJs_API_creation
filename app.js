...
app.post('/create-user', async (request, response, next) => {
  try {
    ...
    // Create the user using the userData object
...
app.post('/create-user', async (request, response, next) => {
  ...
  try {
    ...
    // Return a success response to the client
    return response.status(2000).json({
      message: 'User created successfully',
      user: createdUser
    });
...
*/
/*
let data = await user.findAll({where:{user_id:1}, limit: 10})
...
app.get('/get-user/:userId', async (request, response) => {
  try {
    ...
    if (userData) {
      ...
      // Return a 404 status if user is not found
